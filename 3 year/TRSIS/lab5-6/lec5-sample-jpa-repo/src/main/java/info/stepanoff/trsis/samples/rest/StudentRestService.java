...
public class StudentRestService {
    ...
    @RequestMapping(value = "/{fullName}/{assignmentVariant}/{labCount}/{rating}/{groupNumber}", method = RequestMethod.POST)
public ResponseEntity<Object> add(@PathVariable("fullName") String fullName, 
    @PathVariable("assignmentVariant") int assignmentVariant, 
    @PathVariable("labCount") int labCount, 
    ...
    return ResponseEntity.ok(studentService.add(fullName, assignmentVariant, labCount, rating, groupNumber));
}
    
    @RequestMapping(value = "/search/{fullName}", method = RequestMethod.GET)
    public ResponseEntity<Object> search(@PathVariable("fullName") String fullName, Principal principal) {
    if (principal == null) {
        throw new ForbiddenException();
    }

    Iterable<Student> students = studentService.findByFullName(fullName);
